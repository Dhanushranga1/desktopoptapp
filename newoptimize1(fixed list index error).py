...
# remaining_length_a = fabric_a_length - sum([fabric_a_defects[int(pos)][1] - fabric_a_defects[int(pos)][0] for pos in cut_positions_a])
# remaining_length_b = fabric_b_length - sum([fabric_b_defects[int(pos)][1] - fabric_b_defects[int(pos)][0] for pos in cut_positions_b])
#the below check conditions are to check if the pos in cutposa and cutposb is greater than the length of fabric defects a and fabric defects b 

remaining_length_a = fabric_a_length
for pos in cut_positions_a:
    if int(pos) < len(fabric_a_defects) and len(fabric_a_defects[int(pos)]) > 1:
        remaining_length_a -= fabric_a_defects[int(pos)][1] - fabric_a_defects[int(pos)][0]

remaining_length_b = fabric_b_length
for pos in cut_positions_b:
    if int(pos) < len(fabric_b_defects) and len(fabric_b_defects[int(pos)]) > 1:
        remaining_length_b -= fabric_b_defects[int(pos)][1] - fabric_b_defects[int(pos)][0]
...
visualize_cuts(fabric_a_length+fabric_b_length,fabric_a_defects+fabric_b_defects,cut_positions_a+cut_positions_b, "Combined Fabric")
