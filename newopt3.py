...
if points_per_100_sqm_a > 23:
    ...
    combined_points = sum(defect[2] for defect in fabric_a_defects if defect not in cut_ranges_a) + sum(defect[2] for defect in fabric_b_defects if defect not in cut_ranges_b)
...
if points_per_100_sqm_b > 23:
    ...
    combined_points = sum(defect[2] for defect in fabric_b_defects if defect not in cut_ranges_b) + sum(defect[2] for defect in fabric_a_defects if defect not in cut_ranges_a)
    combined_points_per_100_sqm = (combined_points * 100) / (combined_length * fabric_width)
