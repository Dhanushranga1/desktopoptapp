...
def plot_fabric_sections(canvas_frame, original_length, remaining_length, removed_sections, remaining_sections, width):
    ...
    for start, end in remaining_sections:
        ax.add_patch(patches.Rectangle((start, 1), end - start + 1, width, edgecolor='black', facecolor='lightgreen', label='Remaining Fabric'))
        ax.text((start + end) / 2, 1.5, f'{start}-{end} meters', horizontalalignment='center', verticalalignment='center', fontsize=12, color='black')
...
def main(defects, length, width, threshold_ppms, num_sections, canvas_frame1, canvas_frame2, result_text, info_frame):
    ...
    if original_ppms > threshold_ppms:
        ...
        new_defects, new_length, total_cut_length, removed_sections, remaining_sections = remove_sections(defects, length, width, sections)
        ...
        plot_fabric_sections(canvas_frame2, length, new_length, removed_sections, remaining_sections, width)
        ...
        ttk.Label(info_frame, text="Removed Sections (meters):", font=('Helvetica', 12, 'bold')).pack(pady=5)
        for start, end in removed_sections:
            ttk.Label(info_frame, text=f"From {start} to {end} ({end - start + 1} meters)", font=('Helvetica', 12)).pack(pady=2)

        ttk.Label(info_frame, text="Remaining Sections (meters):", font=('Helvetica', 12, 'bold')).pack(pady=5)
        for start, end in remaining_sections:
            ttk.Label(info_frame, text=f"From {start} to {end} ({end - start + 1} meters)", font=('Helvetica', 12)).pack(pady=2)
    else:
