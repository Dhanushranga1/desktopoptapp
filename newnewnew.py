...
print(f"Interval with highest defect density: {interval_points[max_density_index]}-{interval_points[max_density_index] + 10}")
...
#     [f"{interval_points[i]}-{interval_points[i] + 10}" for i in range(len(interval_points))],
#     [f"{defect_densities[i]:.2f}" for i in range(len(interval_points))]
# ]

# table = mtable.Table(ax, data=table_data)
# ax.add_table(table)
...
# button = plt.Button('Sort', sort_table(1))
# ax.add_artist(button)
...
# # Add a button to sort the table
# button = plt.Button('Sort', lambda event: sort_table(1))
# ax.add_artist(button)
