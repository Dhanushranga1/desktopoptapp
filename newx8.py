...
# Check that max_density_index is a valid index for interval_points and intervals
if max_density_index < len(interval_points) and max_density_index < len(intervals):
    highest_interval = f"{interval_points[max_density_index]}-{interval_points[max_density_index] + intervals[max_density_index // len(intervals)]}"
    print(f"Interval with the highest defect density: {highest_interval}")
else:
    print("Error: max_density_index is out of range")
