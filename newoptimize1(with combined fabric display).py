...
# Function to reduce points and ensure points per 100 sqm <= 23
def reduce_points(defects, length, width, target_points_per_100_sqm):
    ...
    if points_per_100_sqm > target_points_per_100_sqm:
        ...
        for defect in defects_sorted:
            if (total_points - total_removed_points) * 100 / (remaining_length * width) <= target_points_per_100_sqm:
                break
            cut_positions.append((defect[0] + defect[1]) / 2)
            total_removed_points += defect[2]
            remaining_length -= (defect[1] - defect[0] + 1) * (defect[2] / total_points)
            points_per_100_sqm = (total_points - total_removed_points) * 100 / (remaining_length * width)
    return cut_positions, points_per_100_sqm, remaining_length

cut_positions_a, points_per_100_sqm_a, remaining_length_a = reduce_points(fabric_a_defects, fabric_a_length, fabric_width, 23)
cut_positions_b, points_per_100_sqm_b, remaining_length_b = reduce_points(fabric_b_defects, fabric_b_length, fabric_width, 23)
